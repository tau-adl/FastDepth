...
def validate(val_loader, model, epoch, write_to_file=True):
    ...
    f = open(eval_file,"w+")
    f.write("Max_Error  Depth   Min_Error   Depth   \r\n")
    eval_file = output_directory + '/oneframeeval.csv'
    ft = open(eval_file,"w+")
    end = time.time()
    for i, (input, target) in enumerate(val_loader):
        ...
        min_err_ind = np.unravel_index(np.argmin(abs_err, axis=None), abs_err.shape)
        
        min_err_depth = target.data[min_err_ind]
        min_err = abs_err[min_err_ind]
        
        f.write(f'{max_err}  {max_err_depth}   {min_err}   {min_err_depth}   \r\n')
        ...
        if i == 0:
            ...
            if (i == 4*skip):
                new_err = '\n'.join(map(str, abs_err.tolist()))
                ft.write(f'{new_err}')
                new_tar = '\n'.join(map(str, target.tolist()))
                ft.write(f'++++++++\n\r{new_tar}')
