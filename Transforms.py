...
def adjustSaturation(img, saturation_factor):
...
class ColorJitter(object):
    ...
    @staticmethod
    def getParams(brightness, contrast, saturation, hue):
        ...
        if brightness > 0:
            ...
            transforms.append(Lambda(lambda img: adjustBrightness(img, brightness_factor)))

        if contrast > 0:
            contrast_factor = np.random.uniform(max(0, 1 - contrast), 1 + contrast)
            transforms.append(Lambda(lambda img: adjustContrast(img, contrast_factor)))

        if saturation > 0:
            saturation_factor = np.random.uniform(max(0, 1 - saturation), 1 + saturation)
            transforms.append(Lambda(lambda img: adjustSaturation(img, saturation_factor)))

        if hue > 0:
            hue_factor = np.random.uniform(-hue, hue)
            transforms.append(Lambda(lambda img: adjustHUE(img, hue_factor)))
    ...
    def __call__(self, img):
        ...
        transform = self.getParams(self.brightness, self.contrast,
